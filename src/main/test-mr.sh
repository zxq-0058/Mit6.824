...
# （1）什么是mrcoordinator
# mrcoordinator is a program that is part of the MapReduce system. It is responsible for coordinating the processing of
# a MapReduce job by scheduling and coordinating the work of multiple workers. The mrcoordinator program reads input data,
# splits it into smaller pieces, and assigns each piece to a worker to perform a map function. The mrcoordinator also handles
# the intermediate key/value pairs generated by the map function and sends them to the reduce phase to produce the final output.
# It waits for all workers to complete their work and returns the result to the user.
# 说白了就是分配任务
failed_any=0
...
sort mr-out-0 >mr-correct-wc.txt # 标准的答案
rm -f mr-out*

echo '***' Starting wc test.

$TIMEOUT ../mrcoordinator ../pg*txt &  # 等待worker接受任务
pid=$! # 获得进程号
